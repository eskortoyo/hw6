...
window.addEventListener('DOMContentLoaded', async function(event) {
  ...
  // ⬇️ ⬇️ ⬇️
  let apiKey = 'cd0c8c17a1d01dc301b9927b4882275b'
  let url = `https://api.themoviedb.org/3/movie/now_playing?api_key=${apiKey}&language=en-US` 
    console.log(url)
   
  let db = firebase.firestore()
  // let querySnapshot = await db.collection('watched').get('movieID')
  ...
  let movies = json.results
      ...
      for (i = 0; i < json.results.length; i++) {
        // console.log(json.results[i].backdrop_path)
        let movie = movies[i]
        let movieId = movie.id
        let moviePath = movie.poster_path
        ...
        document.querySelector('.movies').insertAdjacentHTML ('beforeend', `
          <div class="movie-${movie.id} w-1/5 p-4 ${opacityLevel}">
            <img src="https://image.tmdb.org/t/p/w500${moviePath}" class="w-full">
            <p class="text-xs text-white">${movieName}</p>
            <a href="#" class="watched-button block text-center text-white bg-green-500 mt-4 px-4 py-2 rounded">I've watched this!</a>
          </div>
        `)

        document.querySelector(` .movie-${movieId} .watched-button`).addEventListener('click', async function(event) {
          console.log(`movie ${movieId} was clicked`)
  ...
  // ⬇️ ⬇️ ⬇️
      ...
      //   let docRef = await db.collection('watched').doc().set({})        
      //   })
      // }
